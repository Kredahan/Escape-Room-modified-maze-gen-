...
public class MazeLoader : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        HuntAndKillMazeAlgorithm ma = new HuntAndKillMazeAlgorithm(mazeCells); //##### Abstract Class Switch Point #####
        ma.CreateMaze();
    ...
    private void InitializeMaze()
    {
        ...
        for (int r = 0; r < mazeRows; r++)
        {
            for (int c = 0; c < mazeColumns; c++)
            {
                ...
                //wall[Random.Range(0, wall.Length)]                                  *size = 2f*  5 * 2 
                // mazeCells[row, column].floor = Instantiate(wall, new Vector3(row * size, -(size / 2f), c * size), Quaternion.identity) as GameObject;
